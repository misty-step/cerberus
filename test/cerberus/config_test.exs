defmodule Cerberus.ConfigTest do
  ...
  describe "reload/1" do
    ...
    test "returns error and preserves state when overrides become invalid", %{server: server} do
      original_personas = Config.personas(server)

      :sys.replace_state(server, fn state ->
        Map.put(state, :overrides, %{"reviewers" => %{"trace" => %{"prompt" => "missing"}}})
      end)

      assert {:error, {:invalid_config, diagnostics}} = Config.reload(server)
      assert Enum.any?(diagnostics, &(&1.path == "reviewers.trace.prompt"))
  ...
  describe "bundled defaults fallback" do
    test "falls back to bundled defaults when repo_root has no config" do
      name = :"config_bad_#{System.unique_integer([:positive])}"
      Process.flag(:trap_exit, true)

      assert {:ok, pid} = Config.start_link(repo_root: "/nonexistent/path", name: name)
      on_exit(fn -> if Process.alive?(pid), do: GenServer.stop(pid) end)

      assert Enum.any?(Config.personas(name), &(&1.id == "trace"))

      trace =
        name
        |> Config.resolved_snapshot(model_tier: :standard)
        |> Map.fetch!(:reviewers)
        |> Enum.find(&(&1.id == "trace"))
