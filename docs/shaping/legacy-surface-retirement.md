...
| `node-scaffolder` | port to Rust | pending | `cerberus-cli init-workflow` now owns deterministic workflow-file scaffolding; `bin/cerberus.js` still owns interactive API-key capture and `gh secret set` compatibility. | Add live secret-setup parity or explicitly keep the Node wrapper before deprecating the entrypoint. |
