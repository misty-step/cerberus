...
| `elixir-http-api` | port to Rust | pending | Rust hosted API compatibility adapter; POST ingress validation and offline `ReviewRequest.v1` acquisition are fixture-backed, while GET status, storage, auth/server wiring, and deployment remain pending. | Implement Rust API service/status fixtures for auth, queue/status responses, store error mapping, and deployment smoke. |
