...
| `elixir-http-api` | port to Rust | pending | Rust hosted API compatibility adapter; POST ingress validation, offline `ReviewRequest.v1` acquisition, service response fixtures, bounded local HTTP listener smoke, and stateful local POST/GET store replay are fixture-backed. Production queue/store lifecycle, deployment smoke, live GitHub acquisition, and reviewer execution remain pending. | Add production Rust queue/store lifecycle and live GitHub acquisition, then define deployment smoke before cutting over from Elixir. |
