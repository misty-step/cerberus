...
| `elixir-verdict-store` | port to Rust | pending | `ReviewRunArtifact.v1` generation plus `FileReviewRunArtifactStore` for immutable, schema-valid receipt persistence. | Add hosted/API persistence fixtures and decide whether SQLite compatibility is required before retiring the Elixir store. |
