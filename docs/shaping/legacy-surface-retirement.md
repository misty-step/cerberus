...
| `node-scaffolder` | port to Rust | pending | `cerberus-cli init` now owns deterministic workflow-file scaffolding, hidden TTY prompt, and `gh secret set`; `bin/cerberus.js` remains for npm package compatibility. | Add Rust npm package parity or explicitly keep the Node wrapper before deprecating the entrypoint. |
