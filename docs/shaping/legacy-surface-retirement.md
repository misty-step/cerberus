...
| `elixir-http-api` | port to Rust | pending | Rust hosted API compatibility adapter; POST ingress validation is fixture-backed, while GET status, storage, auth/server wiring, deployment, and `ReviewRequest.v1` acquisition remain pending. | Implement Rust API service/status fixtures and GitHub acquisition from the ingress pointer into `ReviewRequest.v1`. |
