...
| `node-scaffolder` | port to Rust | pending | `cerberus-cli init` now owns deterministic workflow-file scaffolding plus noninteractive `gh secret set`; `bin/cerberus.js` still owns npm packaging and hidden TTY prompt compatibility. | Add hidden-prompt/package parity or explicitly keep the Node wrapper before deprecating the entrypoint. |
