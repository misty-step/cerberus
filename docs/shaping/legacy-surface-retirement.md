...
| `elixir-http-api` | port to Rust | pending | Rust hosted API compatibility adapter; POST ingress validation, offline `ReviewRequest.v1` acquisition, and service response fixtures for auth, health, queued creation, status reads, not-found responses, and store error mapping are fixture-backed. HTTP server wiring, real queue/store lifecycle, deployment smoke, and live GitHub acquisition remain pending. | Wire a Rust HTTP service around the fixture-backed hosted API contract, then add deployment smoke before cutting over from Elixir. |
