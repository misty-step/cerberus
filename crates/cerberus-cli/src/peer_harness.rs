...
fn run(args: impl IntoIterator<Item = String>, live_mode: bool) -> Result<()> {
    ...
    let artifact = offline_artifact(profile, &input);
    artifact
        .validate()
        .context("offline peer harness artifact failed schema validation")?;
...
fn usage() -> &'static str {
    "usage: cerberus-peer-harness --harness <id> --input <CommandHarnessInput.json> --output <ReviewerArtifact.v1.json> [--profiles <PeerHarnessCommandProfiles.v1.json>]"
}
