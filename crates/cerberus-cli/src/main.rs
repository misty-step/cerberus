...
mod eval_harness;
mod hosted_api_fixture_server;
mod init_workflow;
mod local_review;
mod model_catalog;
mod provider_budget;
use eval_harness::{eval_budget, eval_harness, eval_readiness};
use hosted_api_fixture_server::{run_hosted_api_fixture_server, HostedApiFixtureServerConfig};
use init_workflow::{init, init_workflow};
...
fn main() -> Result<()> {
    ...
    match command.as_str() {
        ...
        "init" => init(args.collect()),
        "init-workflow" => init_workflow(args.collect()),
...
fn hosted_api_ingress_fixture(args: Vec<String>) -> Result<()> {
    let mut body = None;
    let mut out = None;
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            other => bail!("unknown hosted-api-ingress-fixture argument {other:?}"),
        }
    }

    let body_path =
        PathBuf::from(body.context("hosted-api-ingress-fixture requires --body <path>")?);
    let out_path = PathBuf::from(out.context("hosted-api-ingress-fixture requires --out <path>")?);
    let review_id = review_id
        .unwrap_or_else(|| "1".to_string())
        .parse::<u64>()
        .context("hosted-api-ingress-fixture --review-id must be an unsigned integer")?;
...
fn hosted_api_service_fixture(args: Vec<String>) -> Result<()> {
    let mut method = None;
    let mut path = None;
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--method" => {
                method = Some(required_arg(&args, index, "--method")?);
                index += 2;
            }
            "--path" => {
                path = Some(required_arg(&args, index, "--path")?);
                index += 2;
            }
            "--authorization" => {
                authorization = Some(required_arg(&args, index, "--authorization")?);
                index += 2;
            }
            "--api-key" => {
                api_key = Some(required_arg(&args, index, "--api-key")?);
                index += 2;
            }
            "--body" => {
                body = Some(required_arg(&args, index, "--body")?);
            ...
            "--store" => {
                store = Some(required_arg(&args, index, "--store")?);
                index += 2;
            }
            "--out" => {
                out = Some(required_arg(&args, index, "--out")?);
            ...
            other => bail!("unknown hosted-api-service-fixture argument {other:?}"),
        }
    }

    let method = method.context("hosted-api-service-fixture requires --method <GET|POST>")?;
    let path = path.context("hosted-api-service-fixture requires --path <route>")?;
    let api_key = api_key.context("hosted-api-service-fixture requires --api-key <key>")?;
    let out_path = PathBuf::from(out.context("hosted-api-service-fixture requires --out <path>")?);
    remove_stale_file(&out_path)?;

    let body = body
        .map(|body| read_json_value(&PathBuf::from(body)))
        .transpose()?;
    let store = match store {
        Some(path) => read_hosted_api_service_store_fixture(&PathBuf::from(path))?,
        None => HostedApiServiceStoreFixture::default(),
    };
...
fn hosted_api_serve_fixture(args: Vec<String>) -> Result<()> {
    let mut addr = "127.0.0.1:0".to_string();
    let mut api_key = None;
    let mut store = None;
    let mut store_state = None;
    let mut ready_file = None;
    let mut max_requests = 1_u64;
    let mut index = 0;

    while index < args.len() {
        match args[index].as_str() {
            "--addr" => {
                addr = required_arg(&args, index, "--addr")?;
                index += 2;
            }
            "--api-key" => {
                api_key = Some(required_arg(&args, index, "--api-key")?);
            ...
            "--ready-file" => {
                ready_file = Some(PathBuf::from(required_arg(&args, index, "--ready-file")?));
                index += 2;
            }
            "--max-requests" => {
                max_requests = required_arg(&args, index, "--max-requests")?
                    .parse()
                    .context("hosted-api-serve-fixture --max-requests must be an integer")?;
                index += 2;
            }
            other => bail!("unknown hosted-api-serve-fixture argument {other:?}"),
        }
    }

    let api_key = api_key.context("hosted-api-serve-fixture requires --api-key <key>")?;
    if let Some(path) = &ready_file {
        remove_stale_file(path)?;
    }
    let store = match (&store_state, store) {
        (Some(path), _) if path.exists() => read_hosted_api_service_store_fixture(path)?,
        (_, Some(path)) => read_hosted_api_service_store_fixture(&PathBuf::from(path))?,
        _ => HostedApiServiceStoreFixture::default(),
    };

    run_hosted_api_fixture_server(HostedApiFixtureServerConfig {
        addr,
        api_key,
        store,
        store_state,
...
fn hosted_api_dispatch_fixture(args: Vec<String>) -> Result<()> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--artifact-store" => {
                artifact_store = Some(required_arg(&args, index, "--artifact-store")?);
                index += 2;
            }
    ...
    persist_decision_artifact_if_requested(
        &decision,
        artifact_store.map(PathBuf::from),
        "--artifact-store",
    )?;
...
fn github_action_dispatch(args: Vec<String>) -> Result<()> {
    ...
    persist_decision_artifact_if_requested(
        &decision,
        optional_env("CERBERUS_ARTIFACT_STORE").map(PathBuf::from),
        "CERBERUS_ARTIFACT_STORE",
    )?;
...
fn persist_decision_artifact_if_requested(
    decision: &HostedApiDispatchDecision,
    ...
    match (&decision.outcome, &decision.review_run_artifact) {
        (HostedApiDispatchOutcome::Completed, Some(artifact)) => {
            let store = FileReviewRunArtifactStore::new(store_root);
            let path = store.put(artifact)?;
            println!("Cerberus: persisted review artifact to {}", path.display());
        }
        (HostedApiDispatchOutcome::Completed, None) => {
            bail!(
                "Cerberus: {source} requested artifact persistence, but the completed hosted API response did not include review_run_artifact"
            );
        }
        (_, None) if decision.review_run_artifact_error.is_some() => {
            bail!(
                "Cerberus: {source} requested artifact persistence, but the hosted API response included invalid review_run_artifact: {}",
                decision.review_run_artifact_error.as_deref().unwrap_or("invalid artifact")
            );
        }
        _ => {}
    }
...
fn usage() {
    eprintln!(
        concat!(
            ...
            "  cerberus-cli eval-budget --suite <eval-suite.json> --matrix <matrix.json> --readiness <EvalReadinessReport.v1.json> --prompt-tokens <n> --completion-tokens <n> [--retry-count <n>] --out <EvalBudgetEstimateReport.v1.json> [--harness <id>] [--model <id>] [--task <id>]\n",
