defmodule Cerberus.Config.Loader do
  ...
  def load(repo_root, overrides \\ %{}) do
    ...
    case read_defaults(config_path) do
      {:ok, raw_defaults} ->
    ...
    Enum.reduce(entries, {%{}, [], %{}}, fn {id, entry}, {acc, diagnostics, mtimes} ->
      ...
      cond do
        ...
        is_binary(file_path) ->
          ...
          display_path_value = display_path(absolute_path, repo_root)

          case read_asset_content(absolute_path, display_path_value) do
            {:ok, content, file_mtime} ->
              if String.trim(content) == "" do
                {acc,
                 diagnostics ++
                   [
                ...
                mtime = if file_mtime, do: %{absolute_path => file_mtime}, else: %{}
  ...
  defp read_defaults(config_path) do
    case File.read(config_path) do
      {:ok, content} ->
        YamlElixir.read_from_string(content)

      {:error, :enoent} ->
        with {:ok, content} <- Cerberus.BundledAssets.fetch("defaults/config.yml") do
          YamlElixir.read_from_string(content)
        else
          :error -> {:error, :enoent}
        end
  ...
  defp read_asset_content(absolute_path, display_path_value) do
    case File.read(absolute_path) do
      ...
      {:error, :enoent} ->
        case Cerberus.BundledAssets.fetch(display_path_value) do
