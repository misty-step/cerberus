defmodule Cerberus.LLM.OpenRouter do
  ...
  The CLI only accepts OpenRouter credentials here — do not reuse unrelated
  Cerberus environment variables for LLM calls.
  """
  ...
  defp do_call(api_key, params, opts) do
    ...
    model = strip_provider_prefix(params.model)

    payload =
      %{model: model, messages: params.messages, max_tokens: params[:max_tokens] || 4096}
  ...
  defp strip_provider_prefix("openrouter/" <> rest), do: rest
