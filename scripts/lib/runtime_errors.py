...
def classify_api_error_text(text: str, *, runtime_error_class: str | None = None) -> str:
    ...
    if runtime_error_class == "rate_limit" or re.search(
        r"rate.limit|too many requests|retry-after|http[^0-9]*429|error[^0-9]*429",
        lower,
    ):
        return "RATE_LIMIT"
    ...
    if re.search(
        r"incorrect_api_key|invalid_api_key|invalid.api.key|no.cookie.auth|"
        r"no credentials found|authentication failed|unauthorized|"
        r"missing authentication header|http[^0-9]*401",
        lower,
    ):
        return "API_KEY_INVALID"

    if re.search(
        r"exceeded_current_quota|insufficient_quota|insufficient.credits|payment.required|"
        r"quota.exceeded|credits.depleted|credits.exhausted|402|\bbilling\b",
        lower,
    ):
