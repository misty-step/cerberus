...
4) Acceptable third-party refs are full pinned SHAs and full release tags such as `@v1.2.3`. Do not flag `actions/*` or `github/*` actions on semver-style tags such as `actions/checkout@v4`; treat those as lower-risk trusted-provider refs by policy, not as immutable pins.
