...
- Core proof: the branch adds a typed `repo_read` broker, wires it into the shared reviewer runtime, and hardens the boundary after review by blocking symlink escapes, rejecting invalid file ranges, bounding repo-search reads, preserving odd diff paths, and keeping rename entries visible
- Protecting checks:
...
4. Verify the locking tests in `tests/test_repo_read_contract.py`, `tests/extensions/repo-read.test.ts`, and `tests/test_reviewer_profiles.py`, including the new symlink, odd-path, EOF, invalid-range, rename, oversized-file, and `pathPrefix` regressions.

## Execution Evidence

- RED: the new repo-read contract suite initially failed on one assertion and `make validate` caught one lint regression.
- GREEN: the repo-read and github-read extension suite passed with `18/18` tests, the targeted pytest lanes passed, and `make validate` finished with `1738 passed, 1 skipped`.
